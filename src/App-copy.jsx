...
export default function FuelApp() {
  ...
  const [locationEditData, setLocationEditData] = useState(null); // ສຳລັບແກ້ໄຂໂລເຄຊັ໋ນຮ້ານຄ້າ
                ...
                {hasAccess("locationMap") && (
                  <div className="pt-3 md:pt-4 mt-3 md:mt-4 border-t border-gray-100">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center space-x-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl transition text-sm md:text-base mb-1.5 md:mb-2 ${view === "locationMap" ? "bg-orange-100 text-orange-600 font-semibold" : "text-gray-600 hover:bg-orange-50 hover:text-orange-500"}`}
                    >
                    ...
                    {hasAccess("locationList") && (
                      <button
                        ...
                        className={`w-full flex items-center space-x-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl transition text-sm md:text-base mb-1.5 md:mb-2 ${view === "locationList" ? "bg-orange-100 text-orange-600 font-semibold" : "text-gray-600 hover:bg-orange-50 hover:text-orange-500"}`}
                      >
                        <List className="w-4 h-4 md:w-5 md:h-5" />{" "}
                    ...
                    {hasAccess("locationForm") && (
                      <button
                        ...
                        className={`w-full flex items-center space-x-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl transition text-sm md:text-base ${view === "locationForm" ? "bg-orange-100 text-orange-600 font-semibold" : "text-gray-600 hover:bg-orange-50 hover:text-orange-500"}`}
                      >
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden font-lao bg-gray-50/50">
            <header className="h-14 md:h-16 bg-orange-500 shadow-md flex items-center justify-between px-3 md:px-4 lg:px-8 border-b border-orange-600 flex-shrink-0 z-10 text-white">
              ...
              <div className="text-base md:text-xl font-black truncate px-2 flex-1 text-left">
                ...
                {view === "locationMap" && "ແຜນທີ່ຮ້ານຄ້າລູກຄ້າ"}
                {view === "locationList" && "ລາຍຊື່ຮ້ານຄ້າລູກຄ້າ"}
                {view === "locationForm" &&
                  (locationEditData
                    ? "ແກ້ໄຂຂໍ້ມູນສະຖານທີ່"
                    : "ເພີ່ມສະຖານທີ່ຮ້ານຄ້າໃໝ່")}
              </div>
            ...
            <main
              className={`flex-1 overflow-y-auto ${view === "locationMap" ? "p-0" : "p-2.5 sm:p-4 lg:p-8"}`}
            >
              <div
                className={`${view === "locationMap" ? "w-full h-full relative" : "max-w-6xl mx-auto w-full pb-6 md:pb-8"}`}
              >
                ...
                {view.startsWith("location") && hasAccess(view) && (
                  <StoreLocation
                    user={user}
                    currentView={view}
                    setView={setView}
                    locationEditData={locationEditData}
                    setLocationEditData={setLocationEditData}
                  />
                )}
              </div>
            </main>
            {!view.startsWith("locationMap") && <Footer />}
          </div>
...
function StoreLocation({
  ...
  const fetchData = async () => {
    ...
    try {
      if (LOCATION_GAS_URL) {
        ...
        setCustomers(JSON.parse(localStorage.getItem("mockCustomers") || "[]"));
      }
    } catch (err) {
      setError(
  ...
  return (
    <div
      className={`w-full ${currentView === "locationMap" ? "h-full flex flex-col relative overflow-hidden" : ""}`}
    >
      {error && (
      ...
      <div className="flex-1 overflow-y-auto relative">
        ...
        {currentView === "locationForm" && (
          <FormView
            initialData={locationEditData}
            user={user}
            onSave={(data, isEdit) => {
              saveData(isEdit ? "EDIT" : "ADD", data);
              setView("locationList");
            }}
            onCancel={() => setView("locationList")}
            scriptLoaded={scriptLoaded}
          />
        )}
      </div>
  ...
  return (
    <div className="w-full h-full relative">
      ...
      {isAdmin && (
        <button
          onClick={onAddNew}
          className="absolute bottom-[80px] right-[10px] w-14 h-14 flex items-center justify-center bg-orange-500 text-white rounded-full shadow-[0_4px_10px_rgba(0,0,0,0.3)] hover:bg-orange-600 hover:scale-105 transition-all z-10"
      ...
      <button
        ...
        className="absolute bottom-[20px] right-[10px] w-10 h-10 flex items-center justify-center bg-white rounded-sm shadow-[0_1px_4px_rgba(0,0,0,0.3)] text-gray-600 hover:text-orange-500 transition-colors z-10"
...
function FormView({ initialData, user, onSave, onCancel, scriptLoaded }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 max-w-4xl mx-auto w-full">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 space-y-3 shadow-inner">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] md:text-xs text-gray-600 mb-1 font-semibold">
                  ລະຕິຈູດ (Latitude) *
                </label>
                <input
                  required
                  ...
                  value={formData.lat}
                  onChange={handleChange}
                  className="w-full border border-orange-300 bg-white rounded-lg p-2 text-sm outline-none focus:border-orange-500 transition"
                  placeholder="17.xxxx"
                />
              </div>
              <div>
